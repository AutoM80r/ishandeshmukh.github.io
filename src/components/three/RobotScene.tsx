...
const M_CLAW  = new THREE.MeshStandardMaterial({ color: '#3fb950', metalness: 0.30, roughness: 0.42, emissive: '#1a6e24', emissiveIntensity: 0.22 })
...
/* ── pair of axis discs (shoulder / elbow) ────────────────── */
...
function Gripper() {
  return (
    <group>
      {/* mount ball */}
      <mesh material={M_CLAW}>
        <sphereGeometry args={[0.068, 20, 20]} />
      </mesh>
      {/* 3 fingers */}
      {([[-0.062, 0.10,  0.00,  0.00,  0.00, -0.20],
         [ 0.062, 0.10,  0.00,  0.00,  0.00,  0.20],
         [ 0.000, 0.10, -0.06, -0.22,  0.00,  0.00]] as number[][]).map(([px, py, pz, rx, ry, rz], i) => (
        <mesh key={i} material={M_CLAW} position={[px, py, pz]} rotation={[rx, ry, rz]}>
          <boxGeometry args={[0.022, 0.15, 0.020]} />
        </mesh>
      ))}
    </group>
  )
...
/* ── the arm (all geometry + animation) ──────────────────── */
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ flex: 1, position: 'relative', height: 18, display: 'flex', alignItems: 'center' }}>
        <div style={{ position: 'absolute', inset: '0 0', margin: 'auto 0', height: 2, borderRadius: 2, background: 'var(--border)' }} />
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg2)' }}>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <Canvas
          camera={{ position: [3.8, 0.8, 3.8], fov: 44 }}
